...
const navLinks = [
  ...
  { id: 'nav-study-adda', href: '#study-adda', label: 'Learning Adda' },
...
function HeaderContent() {
  ...
  useEffect(() => {
    if (searchParams.get('loggedin') === 'true') {
      ...
      // This is a bit of a hack to remove the query param without a full page reload
      ...
      <nav className="container mx-auto px-6 py-2 flex justify-between items-center">
        ...
        <div className="flex items-center">
          <Link href="/" className="flex items-center gap-2 group">
             <div className="relative">
               <Image src="/file (1).svg" alt="GCC Logo" width={40} height={40} className="rounded-full" priority={true} />
             </div>
        ...
        <div className="flex justify-end">
            <div className="hidden md:block">
              {loading ? <div className="h-[44px] w-[120px] bg-gray-200 rounded-xl animate-pulse" /> : user ? <UserProfile user={user} /> : <LoginButton />}
            </div>
                ...
                <div className="flex justify-between items-center mb-8">
                    ...
                    {navLinks.map((link) => (
                     ...
                     <div className="pt-4" onClick={() => setIsMobileMenuOpen(false)}>
                       {loading ? <div className="h-[44px] w-full bg-gray-200 rounded-xl animate-pulse" /> : user ? <UserProfile user={user} /> : <LoginButton />}
                      </div>
                    </div>
                </SheetContent>
            </Sheet>
...
export function Header() {
  return (
    <Suspense fallback={<div className="h-[68px] w-full" />}>
      <HeaderContent />
    </Suspense>
  )
}
